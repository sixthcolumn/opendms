...
public class LoggingOutInterceptor extends AbstractLoggingInterceptor {
	...
	class LoggingCallback implements CachedOutputStreamCallback {
		...
		public void onClose(CachedOutputStream cos) {
			...
			// if not set, we infer it.
			if (interfaceName != null)
				epriLog.setInterfaceName(getInterfaceName());

			String operation = null;
			QName qn = (QName) message.get(Message.WSDL_OPERATION);
			if (qn != null) {
				operation = qn.getLocalPart();
				if (operation != null) {
					epriLog.setOperation(operation);
					if (interfaceName == null)
						epriLog.setInterfaceName(inferInterface(operation));
				}
			}
			if (operation == null) {
				TreeMap<String, List<String>> m = (TreeMap<String, List<String>>) message
						.get(Message.PROTOCOL_HEADERS);
				if (m != null) {
					List<String> items = m.get("SOAPAction");
					if (items != null) {
						operation = items.get(0);
						epriLog.setOperation(operation);
						if (interfaceName == null)
							epriLog.setInterfaceName(inferInterface(operation));
					}
